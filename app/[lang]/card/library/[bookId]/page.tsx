...
export default function BookDetailPage({ params }: { params: Promise<BookDetailParams> }) {
  const { books, quotes: allQuotes, deleteQuote, deleteBook } = useCardStore();
  ...
  const book = useMemo(() => {
    let bookId = '';
    resolvedParams.then(p => bookId = p.bookId);
    return books.find((b) => b.id === bookId);
  }, [books, resolvedParams]);

  const quotes = useMemo(() => {
    let bookId = '';
    resolvedParams.then(p => bookId = p.bookId);
    return bookId ? allQuotes.filter(q => q.bookId === bookId) : [];
  }, [allQuotes, resolvedParams]);

  if (!book) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <p className="text-fg-muted">{t('card.bookNotFound', { defaultValue: 'Book not found' })}</p>
      </div>
    );
  }
